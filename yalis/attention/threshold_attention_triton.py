...
def thresh_attn_reference(query, key, value, threshold, scale=None, attn_mask=None, enable_gqa=False) -> torch.Tensor:
    ...
    attn_weight = attn_weight.masked_fill(thresh_mask.logical_not(), 0.0)
...
def test():
    ...
    attn_mask = torch.arange(0, T, device=DEVICE).unsqueeze(0).unsqueeze(0) < 1051
    #print (f"Value: {value} \n\n")
    threshold = get_threshold(query, key, value, attn_mask=attn_mask).to(device=DEVICE, dtype=torch.float16).reshape((B, H))
    #print (f"Threshold: {threshold} \n\n")

    # Run the reference implementation
    ref_out = thresh_attn_reference(query, key, value, threshold, attn_mask=attn_mask)
    print (ref_out.shape, ref_out.dtype)

    # Run the Triton implementation
    triton_out = thresh_attn_fused(query, key, value, threshold, attn_mask=attn_mask) 
    ...
    if torch.version.hip is not None and triton.runtime.driver.active.get_current_target().arch == "gfx90a":
        rtol = 1e-2
    assert torch.allclose(ref_out, triton_out, atol=1e-2, rtol=rtol), f"Outputs do not match! {ref_out} vs {triton_out}"


@torch.compile()
def sdpa_attn(query, key, value, attn_mask=None, enable_gqa=False) -> torch.Tensor:
...
config = triton.testing.Benchmark(
                ...
                line_names=["Triton (Percentile: 0)", "Triton (Percentile: 0.5)", "Triton (Percentile: 0.75)", "Triton (Percentile: 0.875)", "Torch"],
