...
class KVCachePolicy(Protocol):
  def allocate(self, slot_ids: List[int], prompt_lengths: torch.Tensor) -> None:
    ...

  def update(self, slot_ids: List[int], n_new_tokens: Union[int, List[int]]) -> None:
    ...
...
class ContiguousKVCachePolicy:
  ...
  def __init__(self, capacity: int, device: Optional[torch.device] = None) -> None:
    self._seq_lens = torch.zeros(capacity, dtype=torch.int32, device="cuda")

  def allocate(self, slot_ids: List[int], prompt_lengths: torch.Tensor) -> Optional[torch.Tensor]:
    if prompt_lengths.numel() == 0 or len(slot_ids) == 0:
      return None
    index = torch.tensor(slot_ids, dtype=torch.int64, device=self._seq_lens.device)
    self._seq_lens.index_copy_(0, index, prompt_lengths.to(dtype=torch.int32, device=self._seq_lens.device))
    return None
...
class PagedKVCachePolicy:
  ...
  def __init__(
    ...
    # Lazy import to avoid import-time errors when the extension isn't built yet.
    from kvcache_manager import KVCacheManager as _CppKVCacheManager  # type: ignore
  ...
  def allocate(self, slot_ids: List[int], prompt_lengths: torch.Tensor) -> None:
    for i, slot in enumerate(slot_ids):
      self._impl.allocate_sequence(int(slot), int(prompt_lengths[i].item()))
  ...
  def update(self, slot_ids: List[int], n_new_tokens: Union[int, List[int]]) -> None:
    if isinstance(n_new_tokens, int):
      ...
      if len(n_new_tokens) != len(slot_ids):
        raise ValueError("n_new_tokens list must match slot_ids length")
  ...
  def lengths(self, slot_ids: List[int]) -> torch.Tensor:
    ...
    index = torch.tensor(slot_ids, dtype=torch.int32, device=all_counts.device)
