...
class KVSlotsManager:
  ...
  def __init__(
    ...
    if paged:
      ...
      ), "paged mode requires max_num_blocks_per_seq, num_blocks, page_block_size"
  ...
  def allocate(
    ...
    assert len(req_ids) == B, "req_ids and prompt_lengths must have the same length"
  ...
  def allocate_for_rows(
    ...
    """
    Allocate KV-cache pages for already-assigned rows with given prompt lengths.
    Does not change row ownership; only manages page assignment.
    """
    if isinstance(rows, torch.Tensor):
  ...
  def view(self, ids: List[int] | List[str], is_request_ids: bool = False) -> Optional[torch.Tensor]:
    """
    Return block_table slice for rows when paged; None for contiguous.
    """
    if is_request_ids:
      slot_ids = [self.slot_allocator.get_slot_id(req_id) for req_id in ids]  # type: ignore[arg-type]
    else:
      slot_ids = ids  # type: ignore[assignment]
    return self.cache_policy.view(slot_ids)  # type: ignore[arg-type]
  ...
  def lengths(self, rows: Union[List[int], torch.Tensor]) -> torch.Tensor:
    ...
    return self.cache_policy.lengths(rows_t.tolist())  # type: ignore[arg-type]
