...
extern "C" __global__ void decode_attn_two_pass_wmma(
  ...
  const int NUM_WARPS_LOCAL = blockDim.x / WARP_SIZE;
  const int TILE_STRIDE     = NUM_WARPS_LOCAL * WARP_SIZE;

  for (int base = warp_id * WARP_SIZE; base < T; base += TILE_STRIDE) {
    int i = base + lane;
    ...
    if (i < T) {
      // p = expf(__half2float(exp_buf[i]) - m) / l; // softmax
      float logit = __half2float(exp_buf[i]); // logits
      p = expf(logit - m) / l; // softmax
      if (p < thr_scalar) {
        p = 0.0f;
      }
      exp_buf[i] = __float2half(p); // store prob
      is_valid = (p > 0.0f);
    }

    // Warp-wide mask of valid entries in this tile
    unsigned mask = __ballot_sync(0xffffffff, is_valid);
    int tile_nnz = __popc(mask);

    if (tile_nnz > 0) {
      int tile_base = 0;
      if (lane == 0) {
        // Reserve 'tile_nnz' contiguous slots in valid_idx
        tile_base = atomicAdd(&valid_cnt, tile_nnz);
      }
      tile_base = __shfl_sync(0xffffffff, tile_base, 0);

      if (is_valid) {
        unsigned lane_mask = mask & ((1u << lane) - 1u);
        int local_rank = __popc(lane_mask);
        int pos = tile_base + local_rank;
        valid_idx[pos] = static_cast<uint16_t>(i);
      }
    }
  }
