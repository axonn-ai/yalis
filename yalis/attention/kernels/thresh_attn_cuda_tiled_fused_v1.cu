...
extern "C" __global__ void decode_attn_gmem_logits_cache_v0(
    ...
    float        scale,
    half*        __restrict__ logits_gmem, // [B*H, T] cached pre-softmax logits (fp16)
  ...
  // Dynamic shared: Q scratch + per-warp output accumulation.
  ...
  // PASS 1: compute logits once, cache to global, and update m/l.
  ...
  // PASS 2: read cached logits, softmax+threshold, and accumulate V.
  ...
  for (int t0 = warp_offset; t0 < T; t0 += BLOCK_N * NUM_WARPS) {
    ...
    int keep = 0;
    if (lane < bs) {
      float logit = __half2float(logits_gmem[bh * (size_t)T + t0 + lane]);
