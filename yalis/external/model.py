...
class Block(nn.Module):
    ...
    def forward(
        ...
        if self.config.parallel_residual:
            ...
            if isinstance(self.mlp, LLaMAMoE):
                x, next_prefetch_weights, next_prefetch_expert_ids = self.mlp(self.norm_2(x), x, prefetch_weights, prefetch_expert_ids)
...
class LLaMAMoE(nn.Module):
    ...
    def forward(self, 
                ...
                prefetch_weights: Optional[torch.Tensor] = None,
                prefetch_expert_ids: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
        B, T, C = (
            x.size()
        )  # batch size, sequence length, embedding dimensionality (n_embd)
        x = x.view(-1, C)  # (B*T, C)
        ...
        y = self.experts(x, router, prefetch_weights, prefetch_expert_ids) # overlap load with expert comp
