...
def matmul_with_one_shot_allreduce(
    ...
    torch.mm(x.view(-1, x.shape[-1]), w.t(), out=out.view(-1, w.shape[0]))
    #print(f"matmul_with_one_shot_allreduce: out_id: {out_id}, inner_group_idx: {inner_group_idx}")
    nvshmem_comm = NVSHMEMCommHandler.get_communicator_from_idx(inner_group_idx)
    #inner_nccl_comm = CommHandler.get_communicator_from_idx(inner_group_idx)
    #inner_nccl_comm.all_reduce(x)
    #torch.distributed.all_reduce(out, op=torch.distributed.ReduceOp.SUM, group=inner_group_name)
...
def _(out, out_id, x, w, inner_group_name, inner_group_idx):
    return torch.empty_like(out)
