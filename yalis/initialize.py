...
def init_distributed(
    ...
    #dist.init_process_group(backend="nccl", rank=rank, device_id=local_device, timeout=timeout) 
    ...
    print (f"[{dist.get_rank()}] Device Prop - {torch.cuda.get_device_properties(torch.cuda.current_device())}")
    if tp_dims is None:
