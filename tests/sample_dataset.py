...
class BenchmarkDataset(ABC):
    ...
    def apply_multimodal_chat_transformation(
        ...
        This method is used for chat models that expect a specific conversation format.
        """
...
class AlpacaDataset(BenchmarkDataset):
    ...
    def sample(
        ...
        **kwargs,
        ...
        base_prompt = (
            "You are a helpful chatbot. Answer as many of the following questions as possible.\n"
        )
        ...
        for _ in range(num_requests):
            data_lines = random.choices(self.data,
                                         k=num_input_lines)
            user_prompt = f"{''.join(data_lines)}"
            #user_prompt = f"{''.join(f"{i}:{s}" for i, s in enumerate(data_lines))}"
            user_msg = [{
                "role": "user",
                "content": user_prompt
            }]
            ...
            #prompt = f"{''.join(f"{i}:{s}" for i, s in enumerate(data_lines))}"
            msg = [{
                "role" : "system",
                "content" : base_prompt
                }, {
...
if __name__ == "__main__":
    ...
    samples = dataset.sample(tokenizer, 8, input_len=16384, return_prompt_formatted=True)
